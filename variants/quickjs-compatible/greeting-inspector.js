// @ls:reload-on-edit
const VERSION = "2026-08-17-quickjs-compatible-ab-test";
const RUNTIME_PROFILE = "quickjs-compatible";

// Capture the body-run payload once. QuickJS fired handlers intentionally receive
// an empty `data` environment, so helpers should close over this body snapshot
// instead of resolving the engine global later from a DOM or modal handler.
const RUN_DATA = data && typeof data === "object" ? data : {};
...
const CHAT_SWITCH_SETTLE_ATTEMPTS = 10;
const CHAT_SWITCH_SETTLE_DELAY_MS = 100;
const LATEST_MESSAGE_RETRY_ATTEMPTS = 10;
const LATEST_MESSAGE_RETRY_DELAY_MS = 100;
...
function getRuntimeEngine() {
  try {
    if (api.utils && typeof api.utils.getEngine === "function") {
      return asText(api.utils.getEngine()) || "unknown";
    }
  } catch {
    // Engine reporting is diagnostic only; LumiScript <2.1 has no getEngine().
  }
...
function getEventName() {
  const explicit = asText(RUN_DATA.__event);

  if (explicit) {
    return explicit;
  }

  if (RUN_DATA.key === "activeChatId") {
    return "SETTINGS_UPDATED";
  }

  if (
    Object.prototype.hasOwnProperty.call(RUN_DATA, "chatId") &&
    !Object.prototype.hasOwnProperty.call(RUN_DATA, "message") &&
    !Object.prototype.hasOwnProperty.call(RUN_DATA, "messageId") &&
    !Object.prototype.hasOwnProperty.call(RUN_DATA, "generationId")
  ) {
    return RUN_DATA.chatId === null ? "CHAT_SWITCHED" : "CHAT_CHANGED";
  }
...
function isManualRun(eventName) {
  return !eventName &&
    !(RUN_DATA.reason === "disabled" || RUN_DATA.reason === "deleted");
}
...
function isTeardown(eventName) {
  ...
  return (
    !eventName &&
    (RUN_DATA.reason === "disabled" || RUN_DATA.reason === "deleted") &&
    (asText(RUN_DATA.scriptId) || asText(RUN_DATA.scriptName))
  );
}

function isActiveChatSettingChange(eventName) {
  return eventName === "SETTINGS_UPDATED" && RUN_DATA.key === "activeChatId";
}

function eventChatId(eventName) {
  return asText(RUN_DATA.chatId) ||
    (isActiveChatSettingChange(eventName) ? asText(RUN_DATA.value) : "");
}

function eventCharacterId() {
  return asText(RUN_DATA.id) ||
    asText(RUN_DATA.character && RUN_DATA.character.id);
}

function isActiveChatClose(eventName) {
  return (
    (eventName === "CHAT_SWITCHED" && RUN_DATA.chatId === null) ||
    (isActiveChatSettingChange(eventName) && !asText(RUN_DATA.value))
  );
}
...
function transitionContentFromEvent() {
  if (typeof RUN_DATA.content === "string") {
    return RUN_DATA.content;
  }

  return chatMessageContent(RUN_DATA.message);
}

function transitionSourceIdFromEvent() {
  return asText(RUN_DATA.messageId) ||
    asText(RUN_DATA.message && RUN_DATA.message.id) ||
    asText(RUN_DATA.generationId);
}
...
function isCurrentRunInteresting(eventName, manualRun) {
  ...
  if (eventName === "MESSAGE_SENT") {
    return isUserMessage(RUN_DATA.message);
  }
...
async function resolveTransitionSource(eventName) {
  if (RUN_DATA.message && isUserMessage(RUN_DATA.message)) {
  ...
  const directExtra = RUN_DATA.message ? RUN_DATA.message.extra : null;
...
async function handleUiAction(action, options = {}) {
  ...
  const actionEngine = getRuntimeEngine();
...
try {
  await main();
  logDebug("run complete", {
    profile: RUNTIME_PROFILE,
    engine: getRuntimeEngine(),
    durationMs: Date.now() - RUN_STARTED_AT,
  });
} catch (error) {
